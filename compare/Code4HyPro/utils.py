...
def adj_acquire(opt, inputs, global_adj, max_len):
    # Num_items x self.step

    adj_all = []
    adj_nd_all = []
    position_embeds = []
    for index in tqdm(range(len(inputs))):
        u_input = inputs[index]
        max_n_node = max_len
        node = np.unique(u_input)
        adj = np.zeros((max_n_node, max_n_node))
        adj_nd = np.zeros((max_n_node, max_n_node))

        for i in sorted(np.arange(1, max_n_node), reverse=True):  # 取索引68,67,...,1
            ...
            if u != v:
                adj_nd[u][v] = global_adj[u_input[i]][u_input[i - 1]]
                adj_nd[v][u] = global_adj[u_input[i - 1]][u_input[i]]
            if u == v or adj[u][v] == 4:
                continue
            adj[v][v] = 1
            if adj[v][u] == 2:
                ...
                adj[u][v] = 2
                adj[v][u] = 3
        adj_dgl = adj_to_dgl(adj, type_graph='dgl')
        position_embed = dgl.random_walk_pe(adj_dgl, opt.step)  # Num_items x self.step
        position_embed = torch.cat((position_embed, torch.zeros((max_n_node - len(position_embed), opt.step))),
                                   0)  # len x 16
        position_embeds.append(position_embed)
        adj_all.append(adj)
        adj_nd_all.append(adj_nd)

    pickle.dump(adj_all, open(opt.path + opt.dataset + '/adj_all' + '.pkl', 'wb'))
    pickle.dump(adj_nd_all, open(opt.path + opt.dataset + '/adj_nd_all' + '.pkl', 'wb'))
    pickle.dump(position_embeds, open(opt.path + opt.dataset + '/position_embeds_all' + '.pkl', 'wb'))

    return adj_all, adj_nd_all, position_embeds


def adj_to_dgl(A, type_graph='dgl'):
